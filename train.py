...
parser.add_argument("--model_names", default=['BTEShapeNet'], type=list, help="'ACM', 'ALCNet', 'DNANet', 'ISNet', 'UIUNet', 'RDIAN', 'RISTDnet'")
...
parser.add_argument("--begin_test", default=300, type=int)
...
parser.add_argument("--log_dir", type=str, default="./otherlogs/BTEShapeNet", help='path of log files')
...
parser.add_argument("--gpu", type=int, default=0, help="GPU id to use")  # 添加用于指定GPU的参数
global opt
...
def train():
    ...
    if opt.resume:
        ...
        ckpt = torch.load('log/IRSTD-1K/BTEShapeNet_537_best.pth.tar')
    ...
    ### Default settings of BTEShapeNet
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        if model_name == 'BTEShapeNet':
            if mode == 'train':
                self.model = BTEShapeNet(config_vit, mode='train', deepsuper=True)
            else:
                self.model = BTEShapeNet(config_vit, mode='test', deepsuper=True)
