...
from model.BTAB import BTAB, odconv_weights_init
from model.SSB import SSB
from model.SPDR import SPDR, CCA
...
class LayerNorm3d(nn.Module):
    ...
    def forward(self, x):
        h, w = x.shape[-2:]  # 通过 x.shape[-2:] 获取输入 x 的最后两个维度 h 和 w，即特征图的高度和宽度。这是为了在后续的操作中能够将维度还原回原始形状。
...
class Block_ViT(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, emb4):
        ...
        emb_all = self.attn_norm(emb_all)  # 1 480 16 16   1 256 480
...
class UpBlock_attention(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        ...
        self.spdr = SPDR(out_channels)  # 添加SPDR模块
        self.nConvs = _make_nConv(in_channels, out_channels, nb_Conv, activation)
...
class Res_block(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if stride != 1 or out_channels != in_channels:  # 输入的尺寸或通道数与输出不一致时，使用一个 1x1 卷积层来调整维度，并进行批归一化。
...
class BTEShapeNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        # 加入 ssb 模块
        self.msce = SSB(out_channels=in_channels * 8)  # 输出通道和输入一致

        # BTAM module (inserted in the encoder after down_encoder4)
        self.btam1 = BTAB(in_channels * 1)
        self.btam2 = BTAB(in_channels * 2)
        self.btam3 = BTAB(in_channels * 4)
        self.btam4 = BTAB(in_channels * 8)
    ...
    def forward(self, x):
        x1 = self.btam1(self.inc(x))  # 32 256 256
        x2 = self.btam2(self.down_encoder1(self.pool(x1)))  # 64 128 128
        x3 = self.btam3(self.down_encoder2(self.pool(x2)))  # 128 64 64
        x4 = self.btam4(self.down_encoder3(self.pool(x3)))  # 256 32 32
